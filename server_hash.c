...
int getServerFileDescriptor(int port, const char *progName) {
  ...
  int status = bind(socketDescriptor, // from socket()
                    (struct sockaddr *)&socketInfo, sizeof(socketInfo));
...
int getPort() {
  ...
  do {
    ...
    char *unused = fgets(buffer, BUFFER_LEN, stdin);
...
void computeAndSendHash(int toClient, const char *filePathCPtr) {
  //  I.  Application validity check:
  int recieved = open(filePathCPtr, O_RDONLY);
  int myMsg;
  if (recieved < 0) {
    myMsg = write(toClient, "Cannot open file", 16);
    return;
  }
  ...
  if (fork() == 0) { 
    ...
    myMsg = dup2(recieved, STDIN_FILENO);
    myMsg = dup2(toClient, STDOUT_FILENO);
    myMsg = execl(HASH_APP_PATH, HASH_APP_PATH, "sha512", NULL);
  }
...
void *handleClient(void *vPtr) {
  ...
  while (1) {
    ...
    int convInt;
    int readNetendInt = read(fd, &convInt, sizeof(int));
    length = ntohl(readNetendInt);
    readNetendInt= read(fd, filepath, length);
